...
func (c *CollectedClientData) Verify(storedChallenge string, ceremony CeremonyType, relyingPartyOrigin string) error {
	...
	if !strings.EqualFold(FullyQualifiedOrigin(clientDataOrigin), relyingPartyOrigin) {
		err := ErrVerification.WithDetails(fmt.Sprintf("Error validating origin with --- Non Qualified Origin from client data : %v --- expected value : %s and received value : %s", clientDataOrigin, relyingPartyOrigin, FullyQualifiedOrigin(clientDataOrigin)))
